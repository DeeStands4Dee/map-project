...
public class RegressionTree implements Serializable {
    ...
    /**
     * Restituisce le regole dell'albero come stringa.
     * @return stringa con tutte le regole dell'albero.
     */
    ...
    /**
     * Costruisce ricorsivamente le regole dell'albero come stringa.
     * @param current regola corrente costruita fino a questo nodo.
     * @return stringa con le regole del sottoalbero.
     */
    private String getRules(String current) {
        String result = "";
        if (root instanceof LeafNode) {
            result += current + " ==> Class=" + ((LeafNode) root).getPredictedClassValue() + "\n";
        } else {
            SplitNode splitNode = (SplitNode) root;
            for (int i = 0; i < childTree.length; i++) {
                String info;
                ...
                String condition = current.isEmpty() ? info : current + " AND " + info;
                result += childTree[i].getRules(condition);
            }
        }
        return result;
    }
